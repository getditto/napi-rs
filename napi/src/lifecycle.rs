...
//!    explicitly releases the underlying napi-side resources (napi_refs,
//!    threadsafe-function handles) while v8 is still healthy, so that when
//!    Node enters `Environment::RunCleanup` later there are no leaked
//!    GlobalHandles for V8 to abort on.
//!
